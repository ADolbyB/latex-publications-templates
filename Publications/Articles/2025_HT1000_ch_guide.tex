...
since the principles of operation are all the same.

\medskip
This is a guide to help understand operating procedure and help explain some possibly unknown features 
of the Motorola HT1000s. The actual manual that is screenshot on the next page is 
a free downloadable PDF that is available on \textit{The Internet Archive}. There is a link to this manual as well
as a link to the Radio Service Software User's Manual placed in the ``Links'' section of this document.
...
XxxX's radios operate in the MURS band in the \textit{VHF-Hi} portion of \textit{The Electromagnetic Spectrum}. 
MURS stands for ``Multi Use Radio Service'' and operates under the FCC Part 95 rules, which in XxxX's case is unlicensed 
spectrum, meaning no license is required to use it. This is unlike Amateur (ham) Radio, which operates under 
\textit{FCC Part 97} Rules, and has $5$ different license classes, $2$ of which are \textit{legacy} only licenses.
...
Hi or Lo designation splits the entire VHF band into $2$ parts.
This split happens at $\approx 100\text{ MHz}$. So if your radio (or television) operates anywhere
above $\approx 100\text{ MHz}$, then it operates in the VHF-Hi band. Broadcast antenna 
televisions actually operate as receivers only in \textit{VHF-Lo}, \textit{VHF-Hi} and \textit{UHF}
bands.

% === Page 4 ========================================================================
Something else to mention is that XxxX's radios are \textit{FCC Part 90} certified for use in the
VHF-Hi band, which in the case of XxxX's Motorola HT1000s means $ 136\text{ MHz} $
to $174\text{ MHz}$. Why? Because that is the allocated spectrum for anything operating under
FCC Part 90 rules in the VHF-Hi band. This is why we are allowed to use an FCC Part 90 radio in the MURS band:
the two sets of rules (Part 90 and Part 95) have overlap and the Part 90 operator license exceeds any license class
in the entire FCC Part 95 regulations.

\medskip
So where are the MURS frequency allocations? They can also be called \textit{channel\\allocations},
which is an old television reference. The MURS frequency channel allocations are numbered and named by 
the FCC as follows:
...
important information just like XXxxxXXXX used in his job as \textsc{Production Engineer} at NovTech in Coconut Creek, FL. 
Most of the information XXxxxXXXX needed for his job was:
...
\begin{itemize}
    ...
    \begin{itemize}
        \item This is what the \textsc{Engineer} or \textsc{Technician} uses to identify electrical circuit \\configurations 
            ...
            method of testing if you have more than 1 of the same assembly. XxxX has 4 HT1000s, so if one breaks,
            XXxxxXXXX can take a good one and a broken one, use the service manual and locate and fix the
            ``root cause'' of the problem.
    \end{itemize}
    \item Other useful tools in the service manual XXxxxXXXX would use:
    \begin{itemize}
...
\item \href{https://www.altium.com/altium-designer}{Altium Designer}
    \begin{itemize}
        ...
        \item This is the software we all used at NovTech. XXxxxXXXX used all the diagrams to fix the 
        \textit{embedded computer systems} we manufactured at NovTech.
    \end{itemize}
\item \href{https://www.kicad.org/}{KiCad}
    \begin{itemize}
        ...
        \item XxxxXXxxxX and XXxxxXXXX both have made projects with this software.
    \end{itemize}
\end{itemize}
...
What is considered a ``Channel Group?'' In this particular use case with XxxX's HT1000s, channels $1 - 5$ 
are considered a group by XXxxxXXXX because these channels all have one thing in common: the \textit{PL Tone}. 
...
(Plymouth PD verified by XXxxxXXXX, Falmouth PD verified by XXXXX) used for ``unit ID.'' This data burst 
is decoded by the dispatch console and used typically by the dispatcher when one of the officers (or 
firefighters or EMS, etc.) calls out on the radio. The dispatcher knows immediately who is calling to keep 
transmissions shorter and help keep the channel clear of traffic. Each officer in a squad car will usually 
have two radios: one handheld, and one mobile unit mounted in the vehicle. Each radio can have its own 
unique ID, so the dispatcher knows if the officer is calling from the handheld or the squad car.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item Group A: Scans all MURS channels by channel group of XxxX's custom MURS \\
            Channels when tuned to any of Channels $1 \text{ through }11$
        \item Group B: Scans all $3$ RX only Marine Channels on XxxX's Radios.
                \begin{itemize}
                ...
                \item Marine Channel $22A$
                    ...
                    \end{itemize}
                        ...
                        \end{itemize}
                        \item When tuned to any Marine Channel on XxxX's Radios, the toggle switch can be switched to 
        ...
        \item Group C: Both local NOAA Weather Radio stations.
            \begin{itemize}
                \item Scan is deactivated on XxxX's $2$ NOAA Weather channels. This is because they are 
...
\begin{itemize}
    ...
    \item C: Channel scan function just like on XxxX's radios.
\end{itemize}
...
\begin{center}
    \item \subsection{\LaTeX\ and Mathematical Equations}
\end{center}

Here are some general math equations, which is why so many people use \LaTeX\ for writing books, doing 
math equations, and creating documents. Here are some general examples which look just like math textbooks:
...
Save the best for last, this is Calculus. Without Calculus, discovered by Isaac \\ \indent Newton and Gottfried
Leibniz, we would still be living with late \textbf{1700s} technology!
\begin{equation}
    \int \limits_a^b f(x) \, \mathrm{d}x = \lim \limits_{x \to \infty} \sum \limits_{k=1}^{n} f(x_k) \cdot \Delta x
\end{equation}

\medskip
Newton and Leibniz were incredibly intelligent. They were like the ``Elon Musks'' of their 
time, which is the Late 1700s.
...
\item \textit{carrier squelch}: When the radio is not receiving any traffic, it remains muted or ``squelched'' so 
    ...
    most of XXxxxXXXX's radios are set to receive in carrier squelch mode so that ANY station calling in with any
