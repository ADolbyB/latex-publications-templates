...
an Embedded Computer Systems OEM in South Florida. Most of the information the author needed for his job was:
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item The layout makes for good use of an \textit{ohmmeter}, to \textit{check continuity} from one place on the PCB 
            to another, ensuring signal integrity. Checking continuity means expecting and measuring a 0 ohm 
            \textit{short} reading on the ohmmeter from one side of a trace to the other.
        \item The layout diagrams describe where everything is and what it looks like in real life, layer by layer if 
            you held any of the components or sub-assemblies in your hand and looked at them. In today's subscription 
            based services, like Altium Designer, each of these layers can be inspected on the computer screen and 
            highlighted as needed for any circuit on the PCB. These modern solutions to old paper manuals are discussed
            on the next page.
    \end{itemize}
    ...
    \begin{itemize}
        \item This is what the \textsc{Engineer} or \textsc{Technician} uses to identify electrical circuit \\configurations 
            for testing. When the circuit configuration is known, we can \\conduct tests and voltage measurements with a 
            ``good'' radio, then compare that result with a ``suspect'' or ``defective'' radio. This is a very effective
            method of testing if there are more than 1 of the same assembly. This client was given 4 HT1000s, so if one breaks,
            any radio repair facility can take one good unit and one broken unit, and use the service manual to locate and 
...
some of these Web Tools. Some are very, very expensive but usually have free trials for a month or so.
...
web tools or downloadable subscription based programs such as:
...
These programs are all called ``EDAs'': Electronic Design Automation tools. They are far superior to drawing 
everything on paper and snail mailing it all like it was years ago.
