...
under FCC Part 90 rules in the VHF-Hi band. Lower than 136 MHz is used for aircraft \textit{(FCC Part 87)}, and above
174 MHz is used for broadcast television (FCC Part 73). 

\medskip 
Also note why we are allowed to use an FCC Part 90 radio in the MURS band: the two sets of rules 
...
% === Page 9 ========================================================================
\begin{center}
    \item \subsection{Channel Group 2: Channels 6 - 10}    
\end{center}
...
Note that all of these have the same tone squelch setting of 127.3 Hz, so they are considered a channel group.
Channel 11 is slightly different than the rest of the channels in Group 2, so it is considered its own
channel group. This is discussed next.

\clearpage    
% === Page 10 ======================================================================== 
\begin{center}
    \item \subsection{Channel Group 3: Channel 11}    
\end{center}

\medskip
The ``special'' channel 11 uses the same MURS 5 (Green Dot) frequency as channel 10 with a different PL Tone than the 
previous two groups. Also, at the end of every transmission on channel 11, there is a unique ``chirp'' noise that can 
be heard by the receiving radio. While listening, after the voice audio stops, the transmitting radio sends out a 
little data burst packet which is what many Police Departments used for unit ID. This data burst is decoded by the 
dispatch console and used typically by the dispatcher when one of the officers (or firefighters, EMS, etc.) calls out 
on the radio. The dispatcher knows immediately who is calling to keep transmissions shorter and help keep the channel 
clear of traffic. Each officer in a squad car will usually have two radios: one handheld, and one mobile unit mounted 
in the vehicle. Each radio can have its own unique ID, so the dispatcher knows if the officer is calling from the 
handheld or the squad car.
...
The purpose of having three separate channel operating groups with different tone squelch settings is for versatility. 
Since the HT1000s have 16 available channels, rather than leave some of the channels unprogrammed, the author decided 
to create three separate groups in case of interference. Channel group 1 will typically be sufficient for most 
operation, but there is the possibility of being in a location where someone else is using the same tone frequency on 
their transmitter, since the MURS channels can be used by anyone in the general public. If in the rare instance MURS 
$1 - 5$ are busy with other traffic using the 103.5 Hz PL tone, channel group 2 can be used with a different PL tone 
of 127.3 Hz or channel group 3 with PL tone of 141.3 Hz to block out the unwanted traffic. This is not a perfect 
solution, though. If the other stations are operating close by, their FM carrier signals can still cause unwanted 
interference by blocking your wanted traffic from being heard at all in some circumstances if their FM carrier signal 
is stronger. This can cause receiving operators to ``miss calls.''

\clearpage
% === Page 11 ========================================================================
\begin{center}
    \item \subsection{Channel Group 4: Channels 12 - 16}    
\end{center}

In Channel Group 4, all channels are set to receive (RX) only. Transmitting on these channels is not allowed in 
this case due to the missing \textit{FCC Part 80} type certification on the HT1000 radios, which is needed for 
transmitting on the Marine VHF band. Transmitting on the NOAA Weather Band is never allowed because the beacons 
are \textit{broadcast stations}.
...
since that is the frequency range of operation for the HT1000s. In that band, there are many different services, 
some of which were not discussed:
...
\medskip
Here is a photo from when this author was servicing the HT1000s for reuse. This is a Panasonic CF-27 the author
purchased and built a custom MS-DOS 6.22 \& Windows XP dual boot operating system specifically for surplus 
radios. The Bird 43 wattmeter is equipped with a 5 watt slug for VHF-Hi Band, so full scale on the meter is 5 watts.
The meter is terminated on the output by a small dummy load. The radio connects to the PC via the accessory connector
as shown.
\medskip
\begin{figure}[H]                       % Place image HERE. Note: [H] requires float pkg
    \centering                          % center image, scale to 100% of the text width:
    \includegraphics[width=1.0\textwidth]{./RSS-Service.jpg}
    \caption{Setting power output using the RSS Service menu.} % Place caption BELOW image.
\end{figure}
...
\begin{enumerate}
    ...
    \item \textit{FCC Part 95}: These are the frequency allocations for Personal Radio Services (PRS). Part 95 
    ...
    \item \textit{The Internet Archive}: This is a public archive of various materials available to anyone on 
        ...
        It is located on the web at: \url{https://archive.org/}.
...
% === Page 17 ========================================================================    
    \item \textit{The Electromagnetic Spectrum}: This is the entire spectrum of frequencies in the universe. It
        ...
        in frequency. Here is a link to an excellent visual graphic from NASA which describes 
        \href{https://science.nasa.gov/wp-content/uploads/2023/04/intro_1-1-jpg.webp}{the Electromagnetic Spectrum}.
    ...
    \item \textit{FCC Part 73}: These are the FCC regulations and spectrum allocation for all broadcast services, 
        including AM/FM broadcast radio and over the air (OTA) broadcast television.
    \item \textit{FCC Part 87}: These are the FCC regulations and spectrum allocation for all Aviation \\Services in 
        the United States. It provides rules for licensing, operation, and technical standards of radio stations 
        used in aviation to ensure the safe, efficient, and economical operation of aircraft, as well as the 
        protection of life and property. In the case of VHF, the primary voice communications for aviation
        are allocated from 118.000 MHz $-$ 136.975 MHz using the AM (Amplitude Modulation) mode rather than the
        FM mode which the HT1000s use.
...
\end{center}
    ...
    \item \textit{Bandwidth}: Bandwidth is how much spectrum the radio uses on a specific channel. The FCC
        ...
        via \textit{RSS}, controls the width of this ``lane'' and must be set properly.
% === Page 18 ========================================================================
    ...
    \item \textit{interference}: According to the FCC, interference is defined as: ``The effect of unwanted energy 
        ...
        ``jam'' your signal so the receiving radio gets no signal or a degraded signal. All radio operators have a
        duty to not create interference since it is illegal.
